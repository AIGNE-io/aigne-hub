...
pnpm tsx scripts/analyze-pricing.ts --env "$ENV" --threshold "$THRESHOLD" --json > /tmp/pricing-raw.json 2>&1

# Clean JSON (remove console output)
sed -n '/^\[/,$p' /tmp/pricing-raw.json > /tmp/pricing-clean.json

# Generate HTML report
echo "📝 Generating HTML report..."
node scripts/generate-html-report.mjs /tmp/pricing-clean.json "$OUTPUT_FILE"

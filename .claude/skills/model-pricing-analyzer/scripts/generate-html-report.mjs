...
.del-btn{background:none;border:none;cursor:pointer;font-size:14px;padding:2px 4px;opacity:0.4;transition:all 0.15s;vertical-align:middle;border-radius:4px}
.del-btn:hover{opacity:1}
.del-btn.del-active{opacity:1;background:#fed7d7}
.mrow.del-marked td{background:rgba(229,62,62,.06) !important;text-decoration:line-through;color:#a0aec0}
.mrow.del-marked .mcol,.mrow.del-marked .scol,.mrow.del-marked .ck-col{text-decoration:none;color:inherit}
.mrow.del-marked .mono{color:#a0aec0}
    ...
    <div class="sec" id="sync-panel" style="background:#fff;margin-top:24px">
      ...
      <div class="sec-body" style="padding:20px 24px">
        ...
        <div style="display:flex;gap:10px;margin-bottom:20px;align-items:center">
          <span style="font-size:13px;color:#4a5568">已录入 <strong id="sync-sel-cnt">0</strong> 个 &nbsp;|&nbsp; 未录入 <strong id="sync-um-cnt" style="color:#3182ce">0</strong> 个<span style="font-size:11px;color:#a0aec0;margin-left:2px">(新增)</span> &nbsp;|&nbsp; 待删除 <strong id="sync-del-cnt" style="color:#e53e3e">0</strong> 个</span>
...
var DEL_LS='aigne-pricing-del';
var readSet,umSelSet,delSet,hideRead=false,cf='all',cpSet=new Set();
var op=null;

function initEventHandlers(){
  readSet=new Set(JSON.parse(localStorage.getItem(LS_KEY)||'[]'));
  umSelSet=new Set(JSON.parse(localStorage.getItem(UM_LS)||'[]'));
  delSet=new Set(JSON.parse(localStorage.getItem(DEL_LS)||'[]'));
  ...
  // mark-deprecated toolbar button removed — deletion now via per-row toggle + sync

  // Init delete buttons — restore saved state
  document.querySelectorAll('.del-btn').forEach(function(btn){
    var k=btn.dataset.rk;
    if(delSet.has(k)){btn.classList.add('del-active');btn.innerHTML='❌';btn.title='取消删除';applyDelStyle(k,true)}
  });

  // Delete button delegation — toggle mark-for-deletion (like checkbox)
  document.addEventListener('click',function(e){
    var btn=e.target.closest('.del-btn');if(!btn)return;
    var rk=btn.dataset.rk;if(!rk)return;
    var marking=!delSet.has(rk);
    applyDel(rk,marking);
  });
...
function getDelSelected(){return new Set(delSet)}
function applyDelStyle(key,on){document.querySelectorAll('tr.r1[data-key="'+key+'"]').forEach(function(r1){r1.classList.toggle('del-marked',on);var r2=r1.nextElementSibling;if(r2&&r2.classList.contains('r2'))r2.classList.toggle('del-marked',on)});document.querySelectorAll('.del-btn[data-rk="'+key+'"]').forEach(function(btn){if(on){btn.classList.add('del-active');btn.innerHTML='❌';btn.title='取消删除'}else{btn.classList.remove('del-active');btn.innerHTML='🗑';btn.title='标记删除'}})}
function applyDel(key,on){if(on)delSet.add(key);else delSet.delete(key);localStorage.setItem(DEL_LS,JSON.stringify([...delSet]));applyDelStyle(key,on);updDelCnt()}
function updDelCnt(){var el=document.getElementById('sync-del-cnt');if(el)el.textContent=delSet.size}
...
async function doSync(isDryRun){
  ...
  var cntMsg=selectedEntries.length+(selectedUm.length?' + '+selectedUm.length+' 新增':'')+(delEntries.length?' + '+delEntries.length+' 删除':'');
  showStatus(isDryRun?'正在预览 '+cntMsg+' ...':'正在执行同步...','#ebf8ff','#2b6cb0');
...
(async function(){
  try{
    ...
    // 4. Official pricing (tier1 providers) — fetch remote first, fallback to embedded
    setStep('op','active');

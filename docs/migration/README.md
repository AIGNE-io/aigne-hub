...
| 13 | AiCredentials migrate "successfully" but every provider call reports `密钥不对` / decryption failure | The `BLOCKLET_APP_EK` generated when you registered the new Cloudflare instance is **not** the `BLOCKLET_APP_EK` that encrypted the source rows — a fresh EK will never decrypt historical ciphertext. Brute-forcing KDF/salt combinations wastes time. NB: `APP_SK` and `APP_EK` are **two independent random values** generated by Blocklet Server at instance init time; EK is NOT derived from SK, so you cannot reconstruct one from the other | Recover the **historical** EK from the source host: `sudo cat /data/.blocklet-server/tmp/docker/<APP_PID>/ai-kit/docker-env-blocklet-*-ai-kit`, then run `docs/migration/scripts/reencrypt-credentials.mjs` to decrypt with the historical EK and re-encrypt with the target `CREDENTIAL_ENCRYPTION_KEY`. The historical EK is a transient input to the migration script only — it MUST NOT be written to a Worker Secret or persisted in any config file, because the CF worker never reads `BLOCKLET_APP_EK` at runtime |
...
| 23 | Operator keeps a `.input.<env>.env` / `wrangler.<env>.local.toml` with plaintext `APP_SK=0x...`, `CREDENTIAL_ENCRYPTION_KEY=...` on disk and sources it before every deploy | These files were originally introduced as "local operator working copies" under `.gitignore`, which stopped them from being committed but NOT from leaking via editor history, cloud sync, or a stolen laptop. Secrets have no legitimate reason to live on disk past a single `wrangler secret put` call | Secrets must go directly from a trusted source (password manager, HSM, or interactive prompt) into Cloudflare's secret store. Never persist them to disk: `printf '%s' "$(op read op://Prod/aigne-hub/APP_SK)" \| wrangler secret put APP_SK -c cloudflare/wrangler.staging.toml`. If you absolutely need a local shell session with secrets exported (e.g. for a one-shot migration script), set them in the shell only (`read -s APP_SK` or `export APP_SK="$(op read ...)"`) and confirm with `history -c` afterwards. The only secret-bearing file ever allowed on disk is a transient `/tmp/` working file that is `rm`'d before the session ends |

...
export function ProjectStackedChart({
  projects = [],
  trends = [],
  metric = 'credits',
  granularity = 'day',
  ...
  const chartData = trends.map((entry) => {
    ...
    resolvedProjects.forEach((project) => {
      ...
      const value =
        metric === 'credits' ? stats?.totalCredits : metric === 'requests' ? stats?.totalCalls : stats?.totalUsage;

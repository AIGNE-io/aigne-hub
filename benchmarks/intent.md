...
- **Core Concept**: 自写 TypeScript benchmark，两部分测试覆盖对比压测/隔离压测，用 fetch + ReadableStream 采集 TTFB/总耗时/Server-Timing
...
│   ├── comparison.ts       # Part 1: Hub vs 直连 vs OpenRouter 对比 + 阶梯并发压测 + Hub Overhead
│   ├── isolation.ts        # Part 2: 隔离压测（Mock Provider，解析 Server-Timing）
...
- `mock-provider.ts` 是 Part 2 依赖的本地 mock 服务，由 `isolation.ts` 自动启动/关闭

### 2.2 两部分测试的定位

| 测试 | 目的 | 调用目标 | 核心指标 |
|------|------|----------|----------|
| **Part 1: Comparison** | Hub vs 直连/OpenRouter 对比 + 阶梯并发压测 | Direct API + OpenRouter + Hub | TTFB 差值、Hub Overhead、RPS、TTFB 退化曲线 |
| **Part 2: Isolation** | Hub 自身有没有性能阻塞 | Hub（Mock Provider） | Server-Timing 各阶段、Hub overhead |
...
- 两部分统一使用 `realistic` payload，更贴近真实使用场景
...
### 3.6 Hub Overhead 一级指标

Comparison 测试为每个 hub target 计算 `hubOverhead` 字段：

```
hubOverhead = serverTiming.total.pXX - serverTiming.providerTtfb.pXX
```

这个数字代表 Hub 代理层自身的处理开销（不含 provider 等待时间和网络延迟），是团队最关心的指标。

终端输出示例：
```
→ Hub overhead (p50): 5ms (session=0ms, modelSetup=5ms, preChecks=0ms)
```

JSON 报告中 hub target 包含 `hubOverhead` 字段。
...
## 4. Part 1: Comparison — Hub vs 直连 vs OpenRouter + 阶梯并发压测

**目的**: 分析 AIGNE Hub 的实现有没有问题——与直连 provider 和 OpenRouter 对比，量化 Hub 代理层引入的额外延迟。同时通过阶梯并发档位，测试 Hub 在不同负载下的表现。

**核心指标**: TTFB 为主（Hub 开销全在首字节前），总耗时为辅，Hub Overhead（Server-Timing total - providerTtfb）
...
3. 对每个并发档位（默认 `1,5,20,40`），每个 target 持续运行指定时长（默认 15s），收集统计
...
## 5. Part 2: Isolation — 隔离压测（Mock Provider + Server-Timing）
...
COMPARISON_CONCURRENCY_LEVELS=1,5,20,40     # Part 1 并发档位
ISOLATION_DURATION=10000                    # Part 2 每档持续时间（ms）
ISOLATION_CONCURRENCY_LEVELS=1,5,10,25,50,100,200  # Part 2 并发档位
...
# Part 2: 隔离压测（自动启动/关闭 mock provider）
...
两个入口都在执行结束后输出 JSON 到 `benchmarks/results/` 目录：

```typescript
interface BenchmarkReport {
  timestamp: string;
  type: 'comparison' | 'isolation';
  config: Record<string, any>;
  results: any;
}
```

Comparison 报告中 hub target 包含 `hubOverhead` 字段（`serverTiming.total - serverTiming.providerTtfb` 的逐百分位差值）。
...
|----------|--------|-----------|
| 测试结构 | 两部分（对比压测 / 隔离压测） | 各有明确目的，互相补充；原 Stress 与 Comparison 高度重叠已合并 |
| 文件结构 | 4 个源文件 | index + 2 入口 + mock |
| Hub Overhead | Comparison 报告一级指标 | `total - providerTtfb` = Hub 自身开销，团队最关心 |
| Server-Timing | Comparison streaming 中通过 SSE 获取；Isolation 非 streaming 下通过 header 获取 | 两种场景都能采集 |
...
| Part 2 并发上限 | 200 | Mock 无 rate limit，可推到更高 |
...
### Included
- Part 1: Hub vs Direct vs OpenRouter TTFB 对比 + 阶梯并发压测（streaming）+ Hub Overhead 一级指标
- Part 2: 隔离压测 + Server-Timing 全阶段分析（非 streaming，realistic payload）
- Warmup 机制
...
|------|--------|------------|
| Provider rate limit | Part 1 高并发多 429 | 记录事件，报告中标注 |
| API 费用 | Part 1 调用消耗 token | gpt-4o-mini + 短 prompt + 限制迭代 |
| 网络波动 | Part 1 数据不稳定 | 多次迭代 + 百分位统计 + CV |
| Mock 与真实 Provider 行为差异 | Part 2 不能反映真实 streaming 行为 | Part 2 只关注 Hub overhead，不关注 streaming |
| 429 副作用影响后续请求 | Hub 内部 credential 降权，影响后续测试 | 注意观察 429 分布 |
